...
def test_workers_integration(
    ...
    assert set(r.status for r in revisions) == {RS.WAITING}
...
def test_workers_integration_modify_stacks_simple(
    ...
    assert revision_1.status == RS.WAITING
    assert revision_2.status == RS.WAITING
    assert revision_3.status == RS.WAITING
